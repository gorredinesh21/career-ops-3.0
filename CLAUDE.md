# career-ops-3.0 -- AI Job Search Pipeline (LangChain engine)

## Origin & what's new in 3.0

career-ops-3.0 is a derivative of the open-source MIT project
[`career-ops`](https://github.com/santifer/career-ops) by
[@santifer](https://santifer.io) (origin of the scanning core, tracker, modes,
and dashboard). **3.0 moves the "intelligence" from agent-driven prompts into a
coded LangChain.js pipeline** under `lc/`:

| Stage | File |
|-------|------|
| Fetch JD for ATS jobs (Playwright) | `lc/fetch-jd.mjs` |
| Add `min_experience_required`, drop > 3 yrs | `lc/filter-experience.mjs` |
| Rate /5 against every resume (best + which resume) | `lc/rate-jobs.mjs` |
| Generate a tailored resume per job | `lc/gen-resume.mjs` |
| Parse resumes / build GitHub project catalog | `lc/parse-resumes.mjs`, `lc/build-project-catalog.mjs` |
| Orchestrator | `lc/pipeline.mjs` |

LLM backend is **Ollama** (local) or **Gemini** via `LLM_PROVIDER` in `.env`.
Ratings / best-resume / min-experience are written to `data/eval-meta.json` (a
sidecar; the tracker schema is unchanged). See `README.md` for usage.

The notes below document the inherited scanning/dashboard/tracker core, which
still works as before. **It's designed to be made yours** — ask the agent to
change archetypes, modes, scoring, or companies and it edits the files.
