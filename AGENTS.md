...
and dashboard). **3.0 adds a coded LangChain.js pipeline** under `lc/` that
fetches JDs, filters by experience (drop > 3 yrs), rates each job /5 against
multiple resumes (keeping the best + which resume), and generates a tailored
resume per job (picking projects from a GitHub catalog). LLM backend is Ollama
or Gemini via `LLM_PROVIDER`; results land in `data/eval-meta.json`. See
`README.md`.

The notes below document the inherited scanning/dashboard/tracker core.
**It's designed to be made yours** — ask the agent to change archetypes, modes,
scoring, or companies and it edits the files.
